<template>
  ...
  >
    ...
    />
    <v-container
      class="inputBox flex-column d-flex text-center"
    ...
    >
      ...
      <v-row
        justify="center"
        v-bind:align="this.files.length >= 3 ? 'end' : 'start'"
      >
        <v-col
          cols="12"
          class="px-4 pt-auto pb-0 text-center"
          v-if="files.length"
        ...
        >
          <v-row
            class="my-1"
            :justify="this.files.length >= 3 ? 'start' : 'center'"
          >
            <v-col
            ...
            >
              <v-card outlined class="pa-1" color="grey lighten-1" @click.stop>
                <v-row class="flex-no-wrap">
                  <v-col cols="10">
                    <h3 class="text-truncate text-start">
                      {{ filesLimit === 2 ? document[i] : "" }}{{ file.name }}
                    </h3>
                  </v-col>
                  <v-spacer></v-spacer>
                  <v-col cols="auto" justify="end" class="pr-3 pl-0">
                    <v-icon
                      ...
                      @keydown.enter.stop="
                        multiple ? clear(file.name) : clearAll()
                      "
                      >mdi-window-close
                    </v-icon>
                  </v-col>
...
export default {
  ...
  methods: {
    ...
    fileTypeCheck(files) {
      this.message = "";
      const acceptFileType = this.acceptExtension
        .split(",")
        .map((val) => val.trim());
      const re = new RegExp(
        `([a-zA-Z0-9\\s_\\\\.\\-\\(\\):])+(${acceptFileType.join("|")})$`,
        "i"
      );
      ...
      try {
        ...
        if (e === InvalidTypeException) {
          this.message =
            "Nieobsługiwany format pliku: ." + e.name.split(".")[1];
          test = false;
        } else throw e;
      }
      return test;
